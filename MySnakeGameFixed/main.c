...
void init_objects(Game_State* game) {
	...
	if (game->label[5] != NULL) {
		game->label[5] = (char*)malloc(sizeof(char) * 40);
	}
	if (game->label[6] != NULL) {
		game->label[6] = (char*)malloc(sizeof(char) * 40);
	}
	...
	if (strcpy_s(game->label[5], 40, "PRESS   1   FOR   A   RAINBOW   SNAKE") != 0) {
		printf("Failed strcpy_s on label[4] from PRESS   R   FOR   A   RAINBOW   SNAKE");
	}
	if (strcpy_s(game->label[6], 40, "PRESS   2   FOR   A   GREEN   SNAKE") != 0) {
	...
	game->font[0] = TTF_OpenFont("C:\\Users\\bapa7\\Desktop\\VisualStudioStuff\\MySnakeGameFixed\\MySnakeGameFixed\\Fonts\\ARCADECLASSIC.ttf", 40);
	if (game->font[0] == NULL) {
		printf("Failed to open font file: %s", TTF_GetError());
	}

	game->font[1] = TTF_OpenFont("C:\\Users\\bapa7\\Desktop\\VisualStudioStuff\\MySnakeGameFixed\\MySnakeGameFixed\\Fonts\\ARCADECLASSIC.ttf", 20);
	if (game->font[1] == NULL) {
		printf("Failed to open font file: %s", TTF_GetError());
	}
...
bool start_screen(SDL_Window* window, Game_State* game, SDL_Event* event) {
	...
	if (surface == NULL) {
		printf("Failed to render start screen text: %s", TTF_GetError());
	}
	...
	if (game->color[0] == NULL) {
		printf("Failed to create texture of green color label on start screen: %s", SDL_GetError());
	}
